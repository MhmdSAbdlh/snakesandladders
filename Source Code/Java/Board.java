...
public class Board extends JFrame implements KeyListener{
	...
	private void snakesCases() {
		...
		URL snake3 = getClass().getResource("snake3.wav");
		URL snake4 = getClass().getResource("snake4.wav");
		try {
			AudioInputStream audioStream = AudioSystem.getAudioInputStream(snake1);
			Clip clip = AudioSystem.getClip();
			AudioInputStream audioStream2 = AudioSystem.getAudioInputStream(snake3);
			Clip clip2 = AudioSystem.getClip();
			AudioInputStream audioStream3 = AudioSystem.getAudioInputStream(snake4);
		...
		moveThread = new Thread(()-> {
			...
			while(i<=rollN) {
				...
				if(blueY==0 && blueX == 720) {
					playerBlue.setLocation(blueX, blueY);
					blueNaPo.setText(Intro.userX.toUpperCase()+" ("+playerLocation(blueX,blueY)+")");
					endGame(blueX, blueY);
					break;
				}
				if(blueX == 800) {
	...
	private void redMove() {
		moveThread = new Thread(()-> {
			...
			while(i<=rollN) {
				...
				if(redY==0 && redX == 720) {
					playerRed.setLocation(redX, redY);
					redNaPo.setText(Intro.userO.toUpperCase()+" ("+playerLocation(redX,redY)+")");
					endGame(redX, redY);
					break;
				}
				if(redX == 800) {
